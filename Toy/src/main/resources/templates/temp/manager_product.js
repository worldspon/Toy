...
$(document).ready(function () {
    ...
    $(document).on('click', '#reg_btn', function () {
        ...
        else
        {
            ...
            foodprice = $('.product_price');
            foodstock = $('.product_quantity');
            console.log($(foodname[0]).val());

            if (fn_validation(files) && fn_validation(foodname) && fn_validation(foodprice) && fn_validation(foodstock) && window.confirm('작성하신 상품을 등록하시겠습니까?'))
            {
...
function fn_validation(arr) {
    // 검증할 데이터가 존재하는지 판단
        for (let i = 0; i < arr.length; i++)
        {
            ...
            if (arr_value.length < 1 || arr_value==null || arr_value=='')
            {
                console.log(whitereg.test(arr_value));
                alert("빈값!!!!");
                return 0;
            }
        }
